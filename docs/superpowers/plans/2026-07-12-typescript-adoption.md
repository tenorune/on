...
- [x] **Step 1: Flip the ratchet IF AND ONLY IF every remaining `.js` under `js/` and `functions/*.js` carries `// @ts-check` and is clean** — change `"checkJs": false` to `"checkJs": true` in `tsconfig.json` and delete the now-redundant per-file `// @ts-check` pragmas. If ANY file still relies on being unchecked, skip this step and record which files block it. Run the full four-command gate either way.
  > **DONE — session 10, 2026-07-15, commit `314c892` (`chore(ts): enable checkJs`, its own flag-only commit).** The session-5 blockers below were cleared across sessions 6–9 (cheap-tail `@ts-check` pass, then the 5 heavy orchestrators — `groupNav`/`canvas` session 7, `groupContext` session 8, `app`/`following` session 9). Re-measured with the flag true: `npm run typecheck` → 0 errors. Full gate green; `sw.js` hash `knockknock-5a934992eae2` unchanged (flag-only → byte-identical emit). ONE deliberate deviation from this step's text: the now-redundant per-file `// @ts-check` pragmas were LEFT IN PLACE (byte-safe cosmetic cleanup, deferred as an optional follow-up — see `docs/HANDOFF.md`). Full record: HANDOFF session-10 CURRENT STATE block.
  > **(historical) SKIPPED — session 5, 2026-07-14. Precondition NOT met, so `checkJs` stays `false`.** 13 `js/*.js` + 3 `functions/*.js` are still plain unchecked `.js` with no `// @ts-check`; flipping `checkJs:true` would check all of them and fail. BLOCKERS (each needs `@ts-check` + a clean pass first): `js/app.js`, `js/following.js`, `js/groupNav.js`, `js/canvas.js`, `js/groups.js` (the "accepted-`@ts-check`-forever" set — the pragma was never actually added), `js/groupContext.js`, `js/db.js`, `js/telegram.js`, `js/firebase-config.js`, `js/features.js` (FROZEN as `.js`, but a `// @ts-check` comment is safe), `js/about-cta.js`/`js/about-invite.js`/`js/about-telegram.js` (MUST stay `.js`; `@ts-check` is fine); `functions/jest.config.js`, `functions/migrate-presence.js`, `functions/repair-user-groups.js`. These are unconverted core files OUTSIDE Task 6's feature-module scope — a distinct, non-trivial follow-up. Gate re-run green either way (web 1721, functions 358, build clean, typecheck 0).

- [x] **Step 2: Guard-test disposition — keep them all.**
  > **DONE — session 10, 2026-07-15: KEEP ALL, operator-confirmed.** Every test named below still exists and passes; none is subsumed by types (each pins runtime/content/byte/cross-runtime contracts the checker can't see). Explicit review outcome expected by the spec (Phase 5): `time-format-vectors.json` (pins shared behavior), `notify-channel-vectors.json` (same), `tests/name-cap-invariant.test.js` + `tests/idFormats.test.js` (span into `database.rules.json`, which types cannot see), `tests/sharedMirror.test.js` + twin (byte-mirror freshness), `tests/cacheOwner.test.js` drift-scan (content contract, not shape), `tests/featuresFreeze.test.js` (build-script text contract), `tests/presencePredicateParity.test.js` (cross-runtime behavior pin). None of these is subsumed by types; retiring any of them requires an operator decision, not a cleanup commit.

- [x] **Step 3: Document the conventions**
  > **DONE — session 5, 2026-07-14.** The landmine block landed in `docs/HANDOFF.md` and the Tech Stack bullet in `README.md:29`, both matching the text below.
...
- [x] **Step 4: Final gate + commit**
  > **DONE — session 10, 2026-07-15.** Gate green after the flip (typecheck 0 · web 1721 · functions 358 · build clean, `sw.js` hash unchanged); the flip shipped as its own commit `314c892` rather than the combined commit sketched below (conventions had already landed in session 5). Plan CLOSED — all seven tasks complete; re-audited independently 2026-07-15 (fresh-container gate green + static audit of every claim, zero failures).
