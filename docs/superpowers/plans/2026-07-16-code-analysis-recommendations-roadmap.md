...
> ✅ **AUDITED (2026-07-17, post-implementation audit) — spec met.** Two drift notes recorded for the record: (1) the `js/telegram` conversion (`f53907f`) edited `tests/name-cap-invariant.test.js` despite the phase's no-test-edits rule — that test reads the source by literal `fs.readFileSync` path, which the Jest moduleNameMapper cannot absorb, so the rename had to be tracked in the assertion; intent unchanged (same rename-tracking `scripts/gen-wordlist.js` required). (2) The esbuild entry flip `js/app.js → js/app.ts` landed three commits after the `app.ts` rename (`6134ca4`, not `07d0f5d`), so `npm run build` fails across `07d0f5d..43fb982` — the per-task gates (jest + typecheck) were never red; build-bisect caveat only, no action needed.
...
>
> ✅ **AUDITED (2026-07-17, post-implementation audit) — spec met; one interface drift recorded:** `effectiveStatus` shipped as `(primary: StatusInput | null, override: OverrideInput | null)` with local presentation-bearing input types rather than the ambient `PresenceNode`/`StatusOverride` pinned in Task 2.1 (the ambient wire types are deliberately minimal and lack the runtime `statusColor`/`paletteKey` fields), without the `now?` param, and with enabled-but-expired overrides rendering unavailable instead of falling back to primary. Each divergence is documented in `js/status.ts` as matching all four original call sites — which wins by this plan's own Step 1 rule (pin observed site behavior before unifying). Bonus beyond spec: a fifth duplicate merge site (groupContext's dot-go-hint, `343c83b`) was also routed through the selector.
...
> ✅ **SHIPPED (2026-07-17) — single commit `83d2dcd` on `claude/typescript-adoption-phase-0-009izp`.** The task spec below is kept as the audit/design reference (this repo keeps completed roadmap specs in place, per the Phase 2 banner). **What actually landed vs the spec — the delta an auditor should check:** (1) Step 1's "widen include to functions subdirs" added net-zero functions errors, because `functions/*.js` top-level was *already* under the root include since Phase-1 Task 5, and the only genuinely-new functions surface — `functions/_shared/*.js` — is a byte-mirror of the already-checked `shared/*.js`; so the plan's predicted error cluster in `functions/telegram*.js` did **not** materialize. (2) The real new error surface was **`scripts/`** (24 errors: 12 implicit-any params, 7 missing-prop-on-`{}`, 4 index-sig, 1 possibly-undefined), fixed JSDoc-only across `scripts/build.js` (18), `gen-wordlist.js` (3), `dev.js` (2), `gen-icons.js` (1). (3) **One non-pure-annotation touch:** `scripts/dev.js` dropped an unused `const { host, … }` destructure (`host` isn't a property of esbuild's `ServeResult` and was never referenced — logging uses `port`/`lan`); behavior-neutral, in a dev-only untested script. (4) `@types/node@^26.1.1` added as a root devDep; `"types": []` pinned in the root tsconfig — verified immune (root typecheck stayed 0 after the install). (5) CI wiring landed in **both** workflows + a new assertion in `tests/deploy-workflows.test.js`. **Gates at the shipped tip:** `npm run typecheck` 0 · `npm run typecheck:scripts` 0 · `node_modules/.bin/jest` (ROOT) **1753** · `cd functions && npm test` 358 · `npm run test:rules` 71 · `npm run build` clean, SW hash `knockknock-e1daea985482` unchanged · zero `@ts-ignore`/`@ts-expect-error`/`@ts-nocheck`.
