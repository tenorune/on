...
**On-device walkthrough (operator, 2026-07-08):** J#3 verified across all three surfaces on the merged code — Mini App pill refusal ✓, bot `/notifications push` refusal ✓, web permission prompt on switch ✓, and the residual deny case delivers via the bot fallback ✓. The walkthrough surfaced one honest-state gap, fixed below. Still to re-walk: **honest callbacks (B#1)** and the new web pill guards. §38's open items (graduation split-brain window, `rootUpdate` ancestor-overlap helper, mock fidelity) still stand.
