...
**Next likely action (Telegram):** the user was mid-**manual dev testing** (runbook Part A, smoke test A8) — the last three commits (cache-owner guard, link-expunge, unlink-expunge) still need a dev redeploy (`node scripts/dev-build.js` + channel deploy + functions deploy, runbook A3–A5). Then the merge decision. The final review's open Important finding (the Telegram script tag in `index.template.html` and the CSP/frame-ancestors relaxation in `firebase.json` ship regardless of the flag) was **decided 2026-07-04: revert them in the flag-flip commit** — the exact three-file procedure, and the `git revert` that re-arms everything at launch, is documented in `docs/telegram-setup.md` § "Merge prep — the flag-flip commit".
