...
**Most recent work (session 2026-07-07f): wave W2 — Cloud Functions round-trip/atomicity pass.** Branch **`claude/functions-efficiency-w2-4e7qm2`** (off the `t1r1jp` tip `45db5de`), pushed, **not merged**. Functions **207 → 236**, web **1408/1408** — but **nothing on-device verified**. Implemented 14 of the §6 F#1–F#15 findings (sequential RTDB writes → atomic multi-path `update('/')`; N+1 read loops → `Promise.all`; helper dedupe), scope held to efficiency + atomicity with end-states provably unchanged. **F#5 (webhook-reply, response-shape change) skipped**; W1's user-facing bugs left untouched. A whole-branch 8-angle review caught + fixed three real regressions (notifier tokens-read coupling, graduation self-follow canvas dup, group-status write-time timestamp). **Three robustness items flagged OPEN for the operator** (graduation crash-window is a pre-existing split-brain gap W2 narrows but doesn't close; the `update('/')` ancestor-overlap contract is self-enforced at only one of 8 sites; the test mocks can't model RTDB overlap rejection). Full rundown **§37**.

**Prior work (session 2026-07-07e): full-feature analysis of the Telegram adaptation — 66 findings, four fix waves; wave W2 handed to a separate session.** On **`claude/telegram-app-adaptation-t1r1jp`**, commits `ba372c1` (handoff bookkeeping) + `e21e72a` (**`docs/superpowers/2026-07-07-telegram-feature-analysis.md`** — five parallel analysis passes: user journeys, UI consistency, bot conversation, client code, Cloud Functions code; synthesized into a 7-item HIGH tier and four candidate fix waves W1–W4, defined in its §1.8). Analysis only — no product code touched; nothing on-device verified. Web **1408/1408**, functions **207/207** (re-verified on a fresh container). Division of labor (operator's call): **wave W2** (functions round-trips/atomicity, analysis §1.6 + §6) goes to a **separate session on its own branch cut from the `t1r1jp` tip** — W1 and W2 overlap on `functions/telegram.js`/`notifier.js`, so parallel work on one branch would race; **wave W1** (HIGH-tier UX fixes, analysis §1.1) continues in the original session on `t1r1jp`, brainstorm → spec first. Rundown **§36**.
...
**Next likely action:** wave **W1** (HIGH-tier UX, analysis §1.1) — brainstorm → spec → plan in the original session on `t1r1jp`; wave **W2** (functions efficiency, analysis §1.6/§6) in flight in a separate session on its own branch off the `t1r1jp` tip; waves **W3/W4** unscheduled (analysis §1.8). (#283 is deliberately tracked on GitHub only — sessions should not carry it; the notifier alignment `794fee2` is **deployed**.) Merge itself is the maintainer's (flag-flip scripted in `docs/telegram-setup.md` § "Merge prep"). **Deploy landmine:** every push to `dev` **wipes the branch-only Telegram functions** (CI `--force` from `dev`'s source — redeploy per runbook A5) and reverts DB rules, so the Mini App dies at boot until redeployed; deploy deps `TELEGRAM_APP_LINK` (root `.env.local`) / `TELEGRAM_APP_URL` (`functions/.env`) + preview rebuilt. See `docs/telegram-setup.md` A5/A6 + Part A.
...
## 37. Session 2026-07-07f — wave W2: Cloud Functions round-trip/atomicity pass (BUILT; NOT on-device verified)
...
**Next likely action (operator's call):** on-device smoke of the bot commands + link/unlink/graduate round-trips (the DB end-states should be identical; only latency/atomicity changed); decide on F#5; decide whether the two flagged robustness gaps (graduation resume, `rootUpdate` helper) become their own small wave. Merge is the maintainer's (W1 and W2 both touch `functions/telegram.js`/`notifier.js` — expect a merge-order conflict to resolve by hand).
