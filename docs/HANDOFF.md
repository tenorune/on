...
**NOT verified:** the merged combination has had no on-device pass. Per the cheat sheet, the two behaviors to re-walk are the **notifier token-less fallback (J#3)** and the **honest callbacks (B#1)** — both sit exactly where the conflicts were resolved non-trivially. §38's open items (graduation split-brain window, `rootUpdate` ancestor-overlap helper, mock fidelity) still stand.
