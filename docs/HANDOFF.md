...
**Next likely action: on-device verification of this session's work + the still-open stale-inviter-label bug.** Graduation, the "?" info-toast entry, the recovery-modal tweaks, the "Bits n bobs" chip relabel, and the web "Share to Telegram" affordance are all **built and green in jest but unverified in the Telegram webview / real browser.** Deploy the branch `functions/` to the test bot (runbook A5 — **every push to `dev` wipes the branch-only Telegram functions**, CI deploys `--force` from `dev`'s source) and walk: the graduate→reload→"linked" round-trip (incl. ↻-regen retry on a taken uid); the "?" toast on both surfaces; the boot confirmation toast; the desktop-Telegram share caption spacing; and the "Share to Telegram" button placement. **Still open (undecided): the §29 arrival-interstitial stale inviter name** — `sharePersonalInvite` (`js/mycode.js`) reuses an existing active invite and only sets `creatorLabel` (to `telegramFirstName()`) on the auto-create branch, so a stale label ("Pwa" observed instead of "Tenorune") sticks; the proposed fix is to refresh the label to the current TG name on share, **awaiting the maintainer's call** (an `AskUserQuestion` on it was dismissed). Deploy deps unchanged: `TELEGRAM_APP_LINK` (root `.env.local`, client build) / `TELEGRAM_APP_URL` (`functions/.env`) + preview rebuilt+redeployed. See `docs/telegram-setup.md` A6 + Part A caveats.
...
## 30. Session 2026-07-06c — Graduation flow + follow-on UX + web "Share to Telegram" (BUILT; on-device verification pending)
...
**Still open / deferred:**
- **§29 stale inviter name** — unresolved; the proposed refresh-label-on-share fix is awaiting the maintainer's decision (see the top-block "Next likely action").
- **On-device verification** of everything above — jest-green only.
