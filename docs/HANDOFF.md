...
**Next likely action: keep applying on-device smoke-test feedback — testing is NOT finished.** The maintainer is still manually exercising this branch on a real device (A9 in `docs/telegram-setup.md` + ad-hoc) and brings findings in as concrete fixes — TDD for code; the jsdom-invisible surfaces (back button across overlays, vertical-swipe-vs-canvas, call close-confirm, deep-link interstitial, the TG share sheet, the new welcome DM) are exactly what on-device catches. **Expect more rounds; do not treat the branch as done.** Two live deploy dependencies before each test pass: (1) this branch is a **`functions/` + client** change — the branch's `functions/` must be deployed to the test bot (runbook A5), and **every push to `dev` wipes the branch-only Telegram functions** (CI deploys functions `--force` from `dev`'s source); (2) deep-link invites *and* the welcome DM's Open button need `TELEGRAM_APP_LINK` (root `.env.local`, client build) / `TELEGRAM_APP_URL` (`functions/.env`) set and the preview rebuilt+redeployed. See `docs/telegram-setup.md` A6 + Part A caveats.
