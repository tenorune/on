...
- **F#5 (Telegram webhook-reply)** — SKIPPED. It changes the webhook *response shape* (respond to the request with the method JSON instead of a separate `sendMessage` HTTPS call), which needs on-device verification and overlaps W1's callback plumbing. Left for the operator to schedule.
