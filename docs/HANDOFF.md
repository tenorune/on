...
**Next likely action: group-context support for the Telegram bot (5 known gaps).** The bot's commands + the knock-back callback are **Direct-context only** — none understand group scope (per-group `statusOverride`, group membership, `contextGroupId` on knocks). All live in **`functions/telegram.js`** (+ `functions/notifier.js` for the knock payload). Mirror the client's group-status model: `js/db/social.js` (setStatus / `setStatusOverride` / `mergeStatusOverride`), `js/groups.js`, and how the client sets/reads `contextGroupId` on a group knock. TDD (functions Jest — the flat mock seeds whole objects). The five gaps:
1. **Knock-back loses group context.** A group knock's button is `knock:${targetUid}` (`buildNotificationKeyboard` `case 'knock'`/`'availability'`, `telegram.js:15,17`) — no group id. The callback `case 'knock'` → `writeKnock(deps, arg, me)` (`telegram.js:224`) writes a plain Direct knock; `writeKnock` only *preserves* an existing `contextGroupId`, never sets one (`telegram.js:159`). But the notifier already sends `contextGroupId` in the knock payload (`notifier.js:76`). Fix: thread the group id into the button (`knock:targetUid:groupId`) and have `writeKnock` set `contextGroupId` so knock-back lands in the group (matching the client's group knock).
2. **`/knock` can't reach a group-only member.** `/knock <name>` searches only `readFollowing` (Direct `userPrefs/{uid}/following`, `telegram.js:185`). A group co-member you don't follow in Direct isn't found. Fix: also search group memberships (`users/{uid}/groups` → `groups/{gid}/members`), disambiguate by group, set `contextGroupId` on the knock.
3. **`/who` has no group variant.** `/who` lists only available Direct-followed contacts by global presence (`telegram.js:170`). Group availability uses `effectiveAvailable(statusOverride, …)` (already used by `/groups`, `telegram.js:205`). Fix: `/who <group>` (or extend `/groups`) listing members available in that group by their `statusOverride`.
4. **`/status` can't go available in a group.** `/status` writes only global `users/{uid}/presence` (`telegram.js:120`). The app's group availability is a per-group `groups/{gid}/members/{uid}/statusOverride = {enabled, status, availableUntil}` + a `notifyGroupAvailability` fan-out (`notifier.js:125`). Fix: `/status <group> [dur]` sets that member's `statusOverride` (mirror `js/db/social.js`) and triggers the group-availability notification.
5. **`/off` can't go unavailable in a group.** Symmetric: `/off` clears global presence only (`telegram.js:130`). Fix: `/off <group>` disables/clears the per-group `statusOverride`.

Open UX question for the next session: how to name a group on the command line (index from `/groups`? name prefix? the 8-char code?) — decide before building 2–5.
...
- **Next session's target queued** in the top-block "Next likely action": group-context support for the bot (5 gaps).
