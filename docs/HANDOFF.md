...
**Most recent work (sessions 2026-07-02 → 2026-07-05): the experimental Telegram adaptation, now fully smoke-tested on the dev preview channel** — branch `claude/telegram-app-adaptation-t1r1jp` (kept current with `dev`; merges clean), pushed, all reviews clean, **deliberately not merged** — the user is holding it on the dev preview channel for now (maintainer merges per convention; no merge planned soon). A Telegram Mini App (auto sign-in by verifying Telegram's signed `initData` server-side → Firebase custom token; optional phrase-account linking — linking or unlinking **expunges** the temporary Telegram-derived account), a companion bot (webhook Cloud Function, `/start /status /off /who /knock /groups /notifications /help`, inline action buttons on notifications), and a notification channel switch in `sendToUser()` (Telegram bot message when `notifyChannel==='telegram'`, FCM fallback on any failure). Feature flag `TELEGRAM_ENABLED` in `js/features.js` — **true on the branch**; the flag-flip-at-merge procedure (which also reverts the ungated script tag + CSP relaxation) is fully scripted in `docs/telegram-setup.md` § "Merge prep — the flag-flip commit". Implementation rundown **§24**; testing/hardening session rundown **§25**; operator runbooks in `docs/telegram-setup.md`.

**Next likely action (Telegram): new-user onboarding + UX polish.** The full A8 smoke test passed on-device (boot, link + doppelgänger-expunge verified in RTDB, unlink, bot commands, knock-back button, channel toggle), but the first-run experience is unshaped: the bot immediately offers its whole command list to someone who has no idea what KnockKnock is, and the Mini App drops a brand-new user into an empty contact list with no onboarding. Next session: design the Telegram onramp (bot first-contact framing + Mini App first-run guidance) and polish UX for new and existing users. Greenfield design work — start with brainstorming/spec, not code. **Landmine for any dev testing: every push to `dev` deletes the branch-only Telegram functions from the dev project** (CI deploys functions `--force` from `dev`'s source) — recovery is one A5 redeploy; see `docs/telegram-setup.md` Part A caveats.
...
**Next (user's roadmap): Telegram new-user onramp/onboarding + UX polish** — the bot's immediate full command list means nothing to a stranger, and a fresh Mini App user lands in an empty app with no guidance. Design work (brainstorm → spec) before code. Web suite 47 suites / 1265 tests; functions 6 / 149.
