...
**Next likely action: nothing blocking on this branch — awaiting the maintainer's merge to `dev`.** The §29 fix (this session) and the whole 07-06c build are shipped, jest-green (web **1385/1385**, functions **171/171**), and now **on-device verified** in the real Telegram webview / browser. §29 is **closed** (fixed + verified). Watch for a further smoke-test round (this was the fourth). At merge: the flag-flip (`TELEGRAM_ENABLED`, ungated script tag, CSP relaxation) is scripted in `docs/telegram-setup.md` § "Merge prep"; every push to `dev` still **wipes the branch-only Telegram functions** (CI `--force` from `dev`'s source — redeploy per runbook A5) and reverts DB rules, so the Mini App dies at boot until redeployed. Deploy deps unchanged: `TELEGRAM_APP_LINK` (root `.env.local`, client build) / `TELEGRAM_APP_URL` (`functions/.env`) + preview rebuilt+redeployed. See `docs/telegram-setup.md` A5/A6 + Part A caveats.
