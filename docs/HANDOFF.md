...
- **Nav-row flash before group-displayname prompt during new-user invite redemption.** Reported just before this handoff, not fixed. The full new-user flow is: secret-phrase modal → "I've saved it" → group-displayname prompt → land in group context. The `b8bf19e` + `7ff015f` fixes suppressed the empty Direct UI from flashing through, but the persistent `#nav-row` still briefly appears before the displayname prompt. Likely needs to keep the nav row `.hidden` until *after* the displayname step completes for invite-redemption flows. The displayname prompt is the `#group-displayname-screen` overlay; investigate where `initNavRow` and the welcome-flow gating overlap.
