...
**Next likely action: build the Graduation flow (spec §7) — "use the app outside Telegram."** The maintainer has greenlit this as the next project. It gives an unlinked Telegram-derived account a secret phrase, **migrating** it to the phrase-derived uid (`uid = sha256(phrase)`) so it becomes a first-class phrase account — web sign-in "just works", and unlink then behaves like any linked account (rename, **not** a merge; the "no merge" boundary stays uncrossed). Enablers already shipped (recovery-modal extraction `js/recoveryModal.js` with intro/warning/cancel knobs; the reserved `#tg-account-slot`). To build: a new server callable `graduateTelegram({ initData, code })` — verify `initData`, require an **unlinked** mapping (mapping uid == derived uid), `newUid = deriveUid(code)`, require target account non-existent, same rate limiter as `validateRecovery`, then move the account; the Account-section entry point **"I also want to use the app outside of Telegram"**; and the recovery modal wired with the graduation copy (spec §7 has the approved strings) + ↻-regen as the retry on target-uid collision. **⚠️ Decide graduation's success feedback at build time** — the `landing-notice` banner it was originally designed to reuse was **removed** as a bespoke one-off (spec §7 flag); reuse an existing toast/confirm surface or delete the now-caller-less `showLandingNotice`/`stampLanding` + `graduated` copy. Start from `docs/superpowers/specs/2026-07-05-telegram-onboarding-ux-design.md` §7 (+ §2, §9). **Smoke-testing may still surface findings in parallel — the branch is not done;** one is already logged (§29: the arrival interstitial shows a **stale inviter name** for linked-account invites). Keep the same TDD + deploy discipline. Two live deploy dependencies before each test pass: (1) this branch is a **`functions/` + client** change — the branch's `functions/` must be deployed to the test bot (runbook A5), and **every push to `dev` wipes the branch-only Telegram functions** (CI deploys functions `--force` from `dev`'s source); (2) deep-link invites *and* the welcome DM's Open button need `TELEGRAM_APP_LINK` (root `.env.local`, client build) / `TELEGRAM_APP_URL` (`functions/.env`) set and the preview rebuilt+redeployed. See `docs/telegram-setup.md` A6 + Part A caveats.
