...
**G11 + G12 closed on 2026-08-06** by the security audit below. **All nine that
remain are ruled: eight WON'T FIX** (G1, M1, M2, M6, M7, and **M16, M17, M18** —
the last three ruled 2026-08-06) **and one (G3) parked as #302.** The rulings are
stamped in the Weight column below and on each entry.

✅ **M16, M17 and M18 were filed unruled on 2026-08-06 and ruled WON'T FIX the
same day, by the operator.** They were never a queue — the entry text below
already said so — and the ruling closes the question rather than deferring it.
**Raise it before working one, not after**, exactly as with G1/M1/M2/M6/M7.

**So there is nothing unruled on this table again**, which is its normal state.
That does not promote anything: **G3/#302 stays parked and still needs the
...
example of, and the reason the "nothing left to DO" sentence above it went from
dated to false. **The operator ruled all three DO in session**, taking M13's
costed route (b), M15's route (a), and both halves of M14. Their entries below
carry what shipped and what each deliberately does not claim.

⚠️ **What that does NOT mean.** The ledger having no unruled items does not
promote anything: **G3/#302 stays parked and still needs the operator's
explicit in-session go-ahead**, and the five WON'T FIX rulings still hold. A
session that reads this table and finds nothing startable has read it
correctly — that is the normal state of this file, and the answer is to ask,
not to pick the largest parked item.
...
| **M8** | ~~`adoptGroupNames` is unreachable from the browser~~ | `ops/panel.html` | **CLOSED** (`17945c3`) — one tick per colliding group, and ticking re-previews |
...
| **G11** | ~~Any signed-in account could repoint another user's `codeIndex` entry at itself~~ | `database.rules.json` + `js/db/social.ts` | **CLOSED** (2026-08-06, `27013b9`) — **whole-app**, not a panel item. Pre-existing, identical on `main`. Merged to `dev`; **live on dev, undeployed to prod** |
| **G12** | ~~`inviteIndex/{token}` was repointed and released from key sets the attacker controls, under the Admin SDK~~ | `functions/telegram-auth.js`, `functions/ops/merge.js` | **CLOSED** (2026-08-06, `551c1a4`+`c9d503d`) — four sinks, one shared ownership helper. Merged to `dev`; **live on dev, undeployed to prod** |
| **M16** | `.ops-audit/` sits inside the functions deploy archive | `firebase.json`, `ops/server.js:67` | **WON'T FIX** (operator, 2026-08-06) — CI cannot reach it; only a local deploy can |
| **M17** | `esc()` does not escape `'` | `ops/panel.html:64` | **WON'T FIX** (operator, 2026-08-06) — latent, no current sink |
| **M18** | Ops responses carry no `nosniff`; the body reader has no size cap | `ops/server.js:906,969` | **WON'T FIX** (operator, 2026-08-06) — both bounded by the loopback-only boundary |
...
✅ **M13, M14 and M15 were never "judged fine to leave" — they were UNRULED, and
they were RULED AND BUILT on 2026-08-05.** (M13/M14 filed 2026-08-04; **M15**
2026-08-05; all three closed 2026-08-05.) They sat in this table because it is
where minors live, not because the section heading applied to them; M9's row is
the precedent for saying so out loud rather than letting the heading stand as a
verdict. All three came out of deriving `docs/smoke-test-2026-08-04.md` against
the code — the run that found them wrote no test and executed nothing, which is
worth keeping as a method result: they are what re-deriving a prescription
against the code you find turns up, the same lesson the security audit recorded
four times. **M13's promotion is the case in point** — the derivation predicted
the false failure, and running it three weeks of code later reproduced the
predicted path, value and count exactly.

**The section's own test was applied to M13, and the operator overrode it.**
Neither M13 nor M14 affects the correctness of a destructive *write* — the test
this section applies — and on that reading both would have stayed deferred. M13
affects the correctness of the **read-back that certifies** one, and this repo
had already been bitten there: `2dec78c` — the merge verifier reporting a
CORRECT merge as owed — is recorded in `docs/operator-panel-smoke-test.md` next
to the production defects rather than as a tooling nit, on the reasoning that
*a verifier that cries wolf on a correct destructive write is worse than no
verifier, since the operator's next move is to hunt a defect that is not
there.* That precedent is what promoted it, and the ruling was the operator's,
as this file said it had to be. **Carry that forward: "does it affect a
destructive write?" is the test for the DEFAULT, not a ceiling** — the thing
that certifies the write counts too.
...
| **M8** — **CLOSED** | `ops/panel.html` | `adoptGroupNames` is accepted by `server.js:623` and implemented by `merge.js:229-231`, but **`panel.html` never sends it** — both merge buttons post only `{loserUid, survivorUid}` (+`telegramRepoint`). So a `group-member-collision` previewed from the browser always resolves *"survivor's record kept"*, and the loser's per-group `displayName` can never be adopted without POSTing the route by hand. | **CLOSED (2026-08-03, `17945c3`).** One tick per colliding group in the preview dialog, built from the conflict's own `gid` — `Conflict` gained an optional `gid` and the merge conflict helper an `extra` bag, so a path-shape rule never had to live in `panel.html`, the one file with no test harness. Ticking **re-previews** rather than remembering the choice: adoption adds a write path and flips the resolution line, so a remembered tick would show one plan and execute another. Two server cases pin the refusal in both directions (an adoption ADDED after the preview, and one DROPPED). The two merge buttons are now one flow, which is how they drifted into both posting a bare `{loserUid, survivorUid}` in the first place. **Driven in a real browser** (Chromium, canned responses, no credential): unticked reads "survivor's record kept" over 3 paths; ticked reads "loser's displayName adopted" over 4; unticking goes back. That harness is a scratchpad script and deliberately uncommitted — it would drag a playwright dependency into a repo with none, and it stubs the very server it checks. ⚠️ **Two planted violations initially stayed GREEN** and were fixed rather than noted: a source assertion that the page CONTAINS `adoptBlock` passes with its call site deleted, and one that the page mentions `onChange` passes with nothing installing the handler. A defined-but-unrendered control is M8's own shape, one layer over. |
