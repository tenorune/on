...
export function openCreateGroupModal() {
  ...
  const onSubmit = async () => {
    ...
    // Telegram skips the auto-opened invite modal: the group-context roster
    // carries a one-tap deep-link share ("Invite to group") plus an "invite
    // specific people" affordance, so the modal here would just surface the
    // web-URL flow TG deliberately avoids. Web keeps the create-time modal.
