...
jest.mock('../js/db.js', () => ({
  ...
  setGroupsEnabled: jest.fn().mockResolvedValue(undefined),
...
describe('app.js broadcast hygiene with palettes disabled', () => {
  ...
  test('broadcasts palettesEnabled=false without destroying the saved color', async () => {
    ...
    expect(db.setGroupsEnabled).toHaveBeenCalledWith('me', false); // GROUPS_ENABLED:false in this suite
  ...
  test('a group invite link with groups off prompts to enable instead of joining', async () => {
    const invites = require('../js/invites.js');
    invites.extractInviteTokenFromUrl.mockReturnValue('TOK');
    invites.resolveInvitePreview.mockResolvedValue({ scope: 'group', groupName: 'Fam', groupId: 'G1' });
    require('../js/app');
    for (let i = 0; i < 6; i += 1) await new Promise((r) => setTimeout(r, 0));
    // Intercepted: the prompt is shown and the invite is NOT silently redeemed.
    expect(document.querySelector('.confirm-overlay')).not.toBeNull();
    expect(invites.attemptRedeemFromUrl).not.toHaveBeenCalled();

    // Declining dismisses the prompt and still doesn't join.
  ...
  test('a personal invite link with groups off redeems normally (not intercepted)', async () => {
    const invites = require('../js/invites.js');
    invites.extractInviteTokenFromUrl.mockReturnValue('TOK');
    invites.resolveInvitePreview.mockResolvedValue({ scope: 'personal', label: 'Alex' });
    require('../js/app');
    for (let i = 0; i < 6; i += 1) await new Promise((r) => setTimeout(r, 0));
    expect(document.querySelector('.confirm-overlay')).toBeNull(); // no groups prompt
    expect(invites.attemptRedeemFromUrl).toHaveBeenCalled();       // redeemed normally
  });
});
